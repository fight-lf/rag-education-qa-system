'''
 1.文档加载，通过PaddleOCR解析文档（PPT,PDF,WORD,IMG,PNG。。。）
 2.文档切割
    （1）按照切分规则进行切分 -》文件块（父子切分）
     2.1 按照中英文各自符号进行切分：？? .。），需要自己自定义分隔符号
     2.2 按照字符长度切分
     2.3 默认切分方式：\n\n换行符
    （2）语义切割 ，利用文本分类模型（bert中文通用分隔模型），进行切割
 3.文档向量化 -> milvus
'''
...
'''
    文档切割 ： 1. 父子切块，首先把整个文档切分成若干个父块
              2. 再基于每一个父块切分成若干个子块
              3.给每一个子块添加元数据（分类标识，父块内容，父块id）
'''
...
def process_documents(directory_path, parent_chunk_size=conf.PARENT_CHUNK_SIZE,
    ...
    for i, doc in enumerate(documents):
        ...
        parent_splitter_to_use = markdown_parent_splitter if is_markdown else parent_splitter # 选择分割器类型
...
if __name__ == '__main__':
    directory_path = r'D:\python10.4\integrated_qa_system\rag_qa\data\ai_data'
    documents = load_documents_from_directory(directory_path)
    # print(documents)
